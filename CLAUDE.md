...
- Each subcommand is its own class (`command/warp/`, `command/group/`, `command/spawn/`), extending `command.BaseCommand` (`CommandAPICommand register()`), built with `withArguments`/`withOptionalArguments`/`withSubcommand(s)` and named after the literal it implements (`CreateCommand`, `PermissionCommand`, ...). Two nearly-identical leaves (`enable`/`disable`) share an abstract base in their package rather than duplicating the branch-building code. Root command classes (`WarpCommand`, `GroupCommand`, `SpawnCommand`, in `command/` directly) are thin: they call `CommandGroupScanner.scan("xyz.goga221.metabasis.command.<domain>")` to reflectively discover every leaf in their package and attach the results via `.withSubcommands(...)`, adding only their own default/fallback behavior (e.g. `/warp <name>` teleport) directly. Because leaf commands are instantiated via reflection with a no-arg constructor, they can't take services via constructor injection — they read them from the static `Services` accessor (populated once in `MetabasisPlugin.onEnable()`) instead. Shared suggestion providers and completion handlers used by more than one leaf in a domain go in a `<Domain>CommandSupport` class in that domain's command package (e.g. `command.warp.WarpCommandSupport`).
